    ...
    )
{
    ...
    else if (mode == LOCAL_APIC_DELIVERY_MODE_STARTUP)
    {
        ...
        //
        // Let the destionation CPU know the SIPI vector to startup.
        //
        destCpu = &Context->Root->Cpus[destProcessorNum];
    ...
    )
{
    ...
    //
    // We intercetpt #DB only when nested page fault is detected on local APIC
    ...
    )
{
    ...
    //
    // Let the state transtion to GuestStateActive -> GuestStateWaitForSipi if
    ...
    )
{
    ...
    HOST_DEBUG((Context,
                "APIC %d INIT received (to be disgarded = %d)\n",
                Context->Cpu->States.Guest.ApicId,
    ...
    //  2. AP receives #SX VMEXIT and enteres the busy loop in WaitForSipi
    //  3. BSP sends INIT (say by the operating system)
    //  4. BSP sends SIPI
    //  5. AP detects SIPI, and resume the guest executioon with VMRUN
    ...
    // This check disgards (6) and prevents (7).
    //
 ...
 *
 * @details This function detects the conditons where the system is needed and
 *      ready for APIC access intercetion. APIC access interception cannot be
 ...
 *      system hang in some cases. To minimilize the window of interception and
...
/**
 * @brief Handles exception occured during execution of the host by simply
